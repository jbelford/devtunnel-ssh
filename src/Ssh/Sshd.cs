...
internal sealed class Sshd
{
    ...
    public static async Task<Sshd> PrepareAsync(int port, string clientPubKey, bool allowWslRoot = false, CancellationToken ct = default)
    {
        ...
        WriteRestricted(cfg.ConfigOut, cfg.RenderConfig(allowWslRoot));
        return cfg;
    }

    internal string RenderConfig(bool allowWslRoot)
    {
        ...
        b.Append($"HostKey {QuoteConfigValue(HostKey)}\n");
        b.Append($"PidFile {QuoteConfigValue(PidFile)}\n");
        b.Append($"AuthorizedKeysFile {QuoteConfigValue(AuthKeys)}\n");
        ...
        if (SftpServerPath is not null)
            b.Append($"Subsystem sftp {QuoteConfigValue(SftpServerPath)}\n");
        return b.ToString();
    }
